...
def excel_to_markdown(xlsx_path, md_path):
    """
    将单个 xlsx 文件转为 markdown 文件，自动清洗 NaN。
    """
...
def process_single_file(xlsx_file):
    """
    处理单个文件，生成同目录下的 md 文件。
    """
    md_file = os.path.splitext(xlsx_file)[0] + ".md"
    excel_to_markdown(xlsx_file, md_file)
    print(f"已生成: {md_file}")

def process_directory(xlsx_dir):
    """
    递归处理目录下所有 xlsx 文件，生成平行的 md 目录，结构完全对等。
    """
    # 生成平行目录名
    parent_dir = os.path.dirname(os.path.abspath(xlsx_dir.rstrip("/")))
    base_name = os.path.basename(os.path.abspath(xlsx_dir.rstrip("/")))
    ...
    for root, dirs, files in os.walk(xlsx_dir):
        # 计算当前目录在原目录下的相对路径
        rel_path = os.path.relpath(root, xlsx_dir)
        # 在平行 md 目录下创建对应子目录
...
def main():
    if len(sys.argv) != 2:
        print("用法: python tests/test_extractor/test_excel-md_extractor.py <xlsx文件或目录>")
        sys.exit(1)
